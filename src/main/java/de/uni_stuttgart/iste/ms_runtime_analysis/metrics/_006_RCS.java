...
/**
 * Class to calculate the metric RCS
 ...
 */
public class _006_RCS implements MetricInterface {
    ...
    public Metric calculateMetric(DefaultDirectedGraph<Service, DefaultEdge> graph) {
        ...
        Metric metric = new Metric("Relative Coupling of Service", "RCS",
        ...
        if (graph.vertexSet().isEmpty()) {
            ...
            for (Service service : graph.vertexSet()) {
                double value =
                        ((double) graph.outDegreeOf(service) / (double) graph.vertexSet().size());
